...
const FAVORITES_KEY = 'proverbs-favorites';
...
export default function ProverbsHome({ proverbs }: { proverbs: Proverb[] }) {
  ...
  // Initialize Fuse.js for fuzzy search (Myanmar + English)
  const fuse = useMemo(() => new Fuse(allProverbs, {
    keys: ['proverb.original', 'meaning.my.spoken', 'meaning.en'],
    threshold: 0.4, // Lower means more strict, 0.4 allows slight typos
    distance: 100,
  }), [allProverbs]);

  // Filter proverbs instantly on the client side
  const filteredProverbs = useMemo(() => {
    ...
    if (showFavorites) {
      return allProverbs.filter((p) => favorites.includes(p.id));
    }
    ...
    return [];
  }, [allProverbs, search, showFavorites, activeAlpha, favorites, fuse]);
  ...
  const goRandom = useCallback(() => {
    ...
    router.push(`/proverb/${randomProverb.id}`);
  }, [allProverbs, router]);
      ...
      <div className="header">
        <div className="header-top">
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button
              ...
              onClick={goRandom}
              aria-label="ကျပန်းစကားပုံ"
              title="ကျပန်း စကားပုံကြည့်ရန် (Random)"
            >
              <ShuffleIcon />
            </button>
            <button
        ...
        <div className="search-container">
          <input
            ...
            placeholder="စကားပုံ၊ အဓိပ္ပာယ် (မြန်မာ/English) ဖြင့် ရှာဖွေနိုင်ပါသည်..."
            value={search}
      ...
      <div className="words-container">
        {filteredProverbs.length > 0 ? (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', gap: '4px', marginLeft: '12px' }}>
                    ...
                    <button
                      ...
                      style={favorites.includes(p.id) ? { color: '#ffc107' } : undefined}
                    >
          ...
          <div className="empty-state">
            {showFavorites ? 'သိမ်းဆည်းထားသော စကားပုံ မရှိသေးပါ။' : 'ရှာဖွေနေသော စကားပုံ မတွေ့ရှိပါခင်ဗျာ။'}
          </div>
        )}
      </div>
